...
class TemplatesManager
{
                ...
                if (isset($action) && $action == 'save-form') {
                    if (Token::check((Http::post('token')))) {
                        ...
                        if (Filesystem::setFileContent(
                            ...
                            Http::redirect(Http::getBaseUrl() . '/admin/templates/edit?template=' . Http::post('name') . '&type=template');
                        }
                    } else {
                        die('Request was denied because it contained an invalid security token. Please refresh the page and try again.');
                    }
    ...
    public static function getPartialsManager()
    {
        Registry::set('sidebar_menu_item', 'templates');
        ...
        switch (Http::getUriSegment(2)) {
            case 'add':
                ...
                if (isset($create_template)) {
                    if (Token::check((Http::post('token')))) {
                        $file = PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Text::safeString(Http::post('name'), '-', true) . '.php';
            ...
            case 'delete':
                if (Http::get('template') != '') {
                    if (Token::check((Http::get('token')))) {
                        Filesystem::deleteFile(PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::get('template') . '.php');
                        Notification::set('success', __('admin_message_template_deleted'));
                        Http::redirect(Http::getBaseUrl() . '/admin/templates');
                    } else {
                        die('Request was denied because it contained an invalid security token. Please refresh the page and try again.');
            ...
            case 'rename':
                ...
                if (isset($rename_template)) {
                    if (Token::check((Http::post('token')))) {
                        if (!Filesystem::fileExists(PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::post('name') . '.php')) {
                            if (rename(
                                PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::post('name_current') . '.php',
                                PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::post('name') . '.php')
                            ) {
                                Notification::set('success', __('admin_message_template_renamed'));
                                Http::redirect(Http::getBaseUrl() . '/admin/templates');
                            }
            ...
            case 'duplicate':
                if (Http::get('template') != '') {
                    if (Token::check((Http::get('token')))) {
                        Filesystem::copy(PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::get('template') . '.php',
                            PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::get('template') . '-duplicate-' . date("Ymd_His") . '.php');
                        Notification::set('success', __('admin_message_template_duplicated'));
                        Http::redirect(Http::getBaseUrl() . '/admin/templates');
                    } else {
                        die('Request was denied because it contained an invalid security token. Please refresh the page and try again.');
            ...
            case 'edit':
                ...
                if (isset($action) && $action == 'save-form') {
                    if (Token::check((Http::post('token')))) {
                        ...
                        if (Filesystem::setFileContent(
                            ...
                            Notification::set('success', __('admin_message_template_saved'));
                            Http::redirect(Http::getBaseUrl() . '/admin/templates/edit?template=' . Http::post('name') . '&type=partial');
                        }
                    } else {
                        die('Request was denied because it contained an invalid security token. Please refresh the page and try again.');
                    }
                ...
                Themes::view('admin/views/templates/extends/templates/edit')
                    ->assign('template', Filesystem::getFileContent(PATH['themes'] . '/' . Registry::get('settings.theme') . '/views/partials/' . Http::get('template') . '.php'))
                    ->assign('type', ((Http::get('type') && Http::get('type') == 'partial') ? 'partial' : 'template'))
                    ->display();
                break;
            default:

                Themes::view('admin/views/templates/extends/partials/list')
