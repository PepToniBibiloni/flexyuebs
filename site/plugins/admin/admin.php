...
if (Admin::isAdminArea()) {
    ...
    Event::addListener('onCurrentEntryBeforeLoaded', function () {
        ...
        $entries_list= Entries::getEntries('','slug');
        foreach ($entries_list as $entry){
           NavigationManager::addItem('entradas', $entry['title'], $entry['title'], Http::getBaseUrl() . '/admin/entries/?entry=' . strtolower($entry['title']), ['class' => 'nav-link']);
        }
        ...
        NavigationManager::addItem('extends', 'partials', '<i class="fas fa-layer-group"></i>' . __('admin_menu_extends_templates', Registry::get('settings.locale')), Http::getBaseUrl() . '/admin/partials', ['class' => 'nav-link']);
        ...
        NavigationManager::addItem('information', 'infomation', '<i class="fas fa-info"></i>' . __('admin_menu_system_information', Registry::get('settings.locale')), Http::getBaseUrl() . '/admin/information', ['class' => 'nav-link']);

        if (Registry::get('settings.locale') == 'ru') {
            NavigationManager::addItem('help', 'documentation', '<i class="far fa-question-circle"></i>' . __('admin_menu_help_documentation', Registry::get('settings.locale')), '', ['class' => 'nav-link', 'target' => '_blank']);
        } else {
            NavigationManager::addItem('help', 'documentation', '<i class="far fa-question-circle"></i>' . __('admin_menu_help_documentation', Registry::get('settings.locale')), '', ['class' => 'nav-link', 'target' => '_blank']);
        }
...
class Admin
{
    ...
    private static function getAdminArea() : void
    {
        ...
        Http::getUriSegment(1) == 'partials'     and TemplatesManager::getPartialsManager();
