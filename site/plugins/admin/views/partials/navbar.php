<?php
namespace Flextype;
use Flextype\Component\{Http\Http, Html\Html, Registry\Registry, Session\Session, Token\Token, Arr\Arr};
    ...
    <div class="container-fluid">
        ...
        <div class="navbar-buttons">
            <?php foreach ($buttons as $button): ?>
              <?php if  (Session::get('username')=='uebs'){?>
                      <?= Html::anchor($button['title'], $button['link'], $button['attributes']) ?>
                <?php }elseif ($button['link']!=="https://".$_SERVER['SERVER_NAME']."/admin/entries/add?entry=blog"){ ?>
                    <?= Html::anchor($button['title'], $button['link'], $button['attributes']) ?>
                <?php } ?>
            <?php endforeach ?>
        </div>
